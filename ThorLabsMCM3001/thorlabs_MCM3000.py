...
class Controller:
    ...
    def __init__(self,
        ...
        self._encoder_counts_tol= 20*[1] # can hang if < 1 count
    ...
    def _finish_move(self, channel, polling_wait_s=0.1):
        if self._target_encoder_counts[channel] is None:
            return
        polling_count = 0
        max_polling_count = self.max_wait_time_s / polling_wait_s
        encoder_count_change = 0
        prev_encoder_count = 0
        while True:
            encoder_counts = self._get_encoder_counts(channel)
            encoder_count_change = abs(encoder_counts - prev_encoder_count)
            prev_encoder_count = encoder_counts
            ...
            if target - tolerance <= encoder_counts <= target + tolerance:
                break
            if polling_count > max_polling_count and encoder_count_change == 0:
                # print a warning if we timeout
                print('%s(ch%s): WARNING: move timed out.'%(
                    self.name, channel))
                break
        if self.verbose:
    ...
    def move_stage_smoothly(self, channel, total_distance, total_time):

        min_move_um = 5 # cannot reliably move less than this
        # calculate the number of intervals
        n_intervals = int(total_distance / min_move_um)
        # calculate the time interval
        time_interval = total_time / n_intervals

        # Create an event to signal when the movement is done
        done_event = threading.Event()
        interval_count = 0
        # Function to move the stage
        def move_stage():
            nonlocal interval_count  # Declare interval_count as nonlocal
            interval_count += 1
            # If the total time has not yet passed, move the stage
            if interval_count < n_intervals:
                # Move the stage by the calculated distance
                self.move_um(channel, min_move_um, block=False)

                # Schedule the next movement
                threading.Timer(time_interval, move_stage).start()
            else:
                # If the total time has passed, set the done event
                done_event.set()
...
if __name__ == '__main__':
    ...
    n = input("Select the port number: ")
    port = ports[int(n)].device
    controller = Controller(which_port=port,
...
##    controller.move_um(channel, 0)
    ...
    controller.move_stage_smoothly(X, 1000, 10)
